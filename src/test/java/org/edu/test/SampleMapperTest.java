...
public class SampleMapperTest {
	...
	* 프로젝트폴더경로 DbInterface/src/main/webapp/WEB-INF/spring/root-context.xml (Mysql서버접속정보 변경해야 합니다.)
	...
	@Inject
	private IF_SampleMapper mapper;//인터페이스를 실행가능하게 mapper 변수로 지정
	//클래스를 실행변수로 사용시=>IF_SampleMapper mapper = new IF_SampleMapper mapper();
	@Test
	...
	@Test
	public void testUpdaetMember() {
		testSelectMember();
		System.out.println("위에서 수정 전 이름을 확인 하세요");
		
		MemberVO vo = new MemberVO();// 수정은 여러개의 변수값을 변경하기 하기 떄문에
		vo.setUserid("user10");
		vo.setUserpw("4321");
		vo.setUsername("user2");
		vo.setEmail("abc@abc.com");
		mapper.updateMember(vo);
		
		System.out.println("아래는서 수정 후 이름을 확인 하세요");
		testSelectMember();
	}
